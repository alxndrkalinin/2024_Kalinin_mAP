...
assert all(
    [c for c in neg_control_sc_data.columns if c in training_sc_data.columns]
    == neg_control_sc_data.columns
)
...
assert all(
    df.filter(regex="^(?!CP__|DP__)").columns
    == df.filter(regex="^(?!CP__|DP__)").columns
)
